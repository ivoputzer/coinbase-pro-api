...
exports.signatureFor = ({
  path,
  method = String.prototype,
  body = String.prototype
}, {
  npm_config_coinbase_pro_api_key = String.prototype,
  npm_config_coinbase_pro_api_passphrase = String.prototype,
  npm_config_coinbase_pro_api_secret = String.prototype
} = process.env, {
  createHmac
} = crypto) => {
...
exports.headersFor = (options, { assign } = Object, { name, version } = info, { signatureFor } = exports) => {
...
exports.request = (options, { assign } = Object, { request } = https, { hostnameFor, headersFor, toString } = exports) => {
  const hostname = hostnameFor(options) // fixme: this should be done by main script
  const headers = headersFor(options) // fixme: signature is not always mandatory
